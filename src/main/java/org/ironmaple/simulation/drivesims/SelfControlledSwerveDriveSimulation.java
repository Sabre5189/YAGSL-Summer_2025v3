...
public class SelfControlledSwerveDriveSimulation
{
   ...
   *                           meters, and heading in radians). Increase these values to reduce the trust in the vision
   *                           pose measurement.
   */
   ...
   *                               of the robot and provide a chassis speed that has only a dtheta component, the robot
   *                               will rotate around that corner.
   * @param fieldCentricDrive      Whether to execute field-centric drive with the provided speed.
   * @param discretizeSpeeds       Whether to apply {@link ChassisSpeeds#discretize(ChassisSpeeds, double)} to the
   *                               provided speed.
   */
  ...
  public ChassisSpeeds getMeasuredSpeedsFieldRelative(boolean useGyroForAngularVelocity)
  {
    ...
    speeds =
        speeds.toFieldRelative(getOdometryEstimatedPose().getRotation());
    return speeds;
  }
