 ...
 *
 * <p>To visualize these stacks, use {@link #getStackedAlgaePoses()} and {@link #getStackedCoralPoses()} to retrieve
 * the
...
public class ReefscapeCoralAlgaeStack extends GamePieceOnFieldSimulation
{
  ...
  private void simulationSubTick(int subTickCount)
  {
    ...
    if (getPoseOnField().getTranslation().minus(initialPosition).getNorm() > 0.2
        && getLinearVelocity().getMagnitude() > 0.3)
    {
      collapse();
    }
  }
  ...
  {
    if ("Coral".equals(intakeTargetGamePieceType))
    {
      throwAlgaeToGround();
    } else if ("Algae".equals(intakeTargetGamePieceType))
    {
      throwCoralToGround();
    }
  }
