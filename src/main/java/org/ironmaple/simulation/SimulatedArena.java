...
public abstract class SimulatedArena
{
  ...
  public static SimulatedArena getInstance()
  {
    ...
    if (instance == null)
    {
      instance = new Arena2025Reefscape();
    }
   ...
   *
   * @param robotPeriod                 the time between two calls of {@link #simulationPeriodic()}, usually obtained
   *                                    from {@link TimedRobot#getPeriod()}
   * @param simulationSubTicksPerPeriod the number of Iterations, or {@link #simulationSubTick(int)} that the simulation
